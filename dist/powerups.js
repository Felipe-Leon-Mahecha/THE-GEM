...
const POWERUP_ICON_PLACEHOLDER = 'assets/UI/Store/Placeholders/placeholder_store_item.png';
const COIN_PER_GEM = 30;

/** Solo en tienda VIP (en normal aparecen abajo como enlace). */
const POWERUP_VIP_IDS = new Set([
    'teletransporte',
    'monedas_x2',
    'tierra',
    'vida_extra',
...
function powerupPricing(useGems, unlockGems) {
    return {
        useGems,
        unlockGems,
        useCoins: useGems * COIN_PER_GEM,
        unlockCoins: unlockGems * COIN_PER_GEM
    };
}

function isPowerupVipOnly(id) {
    return POWERUP_VIP_IDS.has(id);
}

function getPowerupsForShop(mode = 'normal') {
    if (mode === 'vip') return POWERUPS_DATA;
    return POWERUPS_DATA.filter(powerup => !isPowerupVipOnly(powerup.id));
}

function getPowerupVipTeasers() {
    return POWERUPS_DATA.filter(powerup => isPowerupVipOnly(powerup.id));
}

const POWERUPS_DATA = [
    {
        id: 'proteccion', name: 'Proteccion', color: '#AAE3D8', pricing: powerupPricing(8, 120), upgrade: [0, 8, 13, 20, 30, 48],
    ...
    {
        id: 'fantasma', name: 'Fantasma', color: '#DEAAF0', pricing: powerupPricing(9, 130), upgrade: [0, 9, 15, 24, 36, 58],
    ...
    {
        id: 'stop_time', name: 'Stop Time', color: '#B09C54', pricing: powerupPricing(14, 165), upgrade: [0, 14, 23, 36, 54, 84],
    ...
    {
        id: 'camara_lenta', name: 'Camara Lenta', color: '#D9C938', pricing: powerupPricing(10, 140), upgrade: [0, 12, 19, 30, 45, 70],
    ...
    {
        id: 'vida_extra', name: 'Vida Extra', color: '#DE21C8', pricing: powerupPricing(14, 170), upgrade: [0, 22, 34, 52, 76, 110],
    ...
    {
        id: 'destruccion_letal', name: 'Destruccion Total', color: '#ED2121', pricing: powerupPricing(15, 175), upgrade: [0, 24, 40, 66, 96, 140],
    ...
    {
        id: 'caos', name: 'Caos', color: '#A16603', pricing: powerupPricing(16, 185), upgrade: [0, 34, 55, 86, 128, 190],
    ...
    {
        id: 'iman', name: 'Iman', color: '#FAFF00', pricing: powerupPricing(8, 125), upgrade: [0, 9, 15, 24, 36, 60],
    ...
    {
        id: 'monedas_x2', name: 'Monedas x2', color: '#FFD700', pricing: powerupPricing(14, 170), upgrade: [0, 30, 52, 86, 130, 200],
    ...
    {
        id: 'angel_guardian', name: 'Angel Guardian', color: '#4BC0EB', pricing: powerupPricing(15, 175), upgrade: [0, 28, 48, 78, 118, 170],
    ...
    {
        id: 'teletransporte', name: 'Teletransporte', color: '#3B289E', pricing: powerupPricing(12, 150), upgrade: [0, 16, 28, 0, 0, 0], maxLevel: 3,
    ...
    {
        id: 'sobrecarga', name: 'Sobrecarga', color: '#F97316', pricing: powerupPricing(12, 150), upgrade: [0, 14, 22, 34, 52, 82],
    ...
    {
        id: 'sombra', name: 'Sombra', color: '#2D2D2D', pricing: powerupPricing(12, 155), upgrade: [0, 16, 26, 42, 66, 98],
    ...
    {
        id: 'tierra', name: 'Toque de oro', color: '#8B4513', pricing: powerupPricing(12, 150), upgrade: [0, 18, 30, 48, 72, 108],
    ...
    {
        id: 'burbuja', name: 'Burbuja', color: '#FF6EB4', pricing: powerupPricing(10, 135), upgrade: [0, 12, 19, 30, 45, 70],
...
function savePowerups(inventory) {
    const source = inventory ?? window.powerupsInventory ?? readPowerups();
    const normalized = normalizePowerupsStore(source);
    localStorage.setItem(POWERUP_STORAGE_KEY, JSON.stringify(normalized));
    window.powerupsInventory = normalized;
    return normalized;
}
...
function getPowerupPricing(powerup) {
    if (powerup.pricing) return powerup.pricing;
    const legacyGems = powerup.buy?.gems || 10;
    const legacyCoins = powerup.buy?.coins || legacyGems * COIN_PER_GEM;
    return {
        useGems: legacyGems,
        unlockGems: Math.max(legacyGems * 8, 120),
        useCoins: legacyCoins,
        unlockCoins: Math.max(legacyCoins * 4, legacyGems * COIN_PER_GEM * 8)
    };
}

function getPowerupUseAmount(powerup, source = 'normal') {
    const pricing = getPowerupPricing(powerup);
    if (source === 'vip') return { amount: pricing.useGems, currency: 'gems' };
    return { amount: pricing.useCoins, currency: 'coins' };
}

function getPowerupUnlockAmount(powerup, source = 'normal') {
    const pricing = getPowerupPricing(powerup);
    if (source === 'vip') return { amount: pricing.unlockGems, currency: 'gems' };
    return { amount: pricing.unlockCoins, currency: 'coins' };
}

function unlockPowerup(id, source = 'normal') {
    const powerup = getPowerupById(id);
    if (!powerup) return false;
    const inventory = readPowerups();
    const item = inventory[id];
    if (!item || item.desbloqueado) return false;
    const { amount, currency } = getPowerupUnlockAmount(powerup, source);
    ...
    if ((item.usos || 0) <= 0) item.usos = 0;
    savePowerups(inventory);
    window.playSfx?.('vipBuy', 0.85);
    refreshPowerupShopViews({ id, mode: source, level: item.nivel || 1 });
    return true;
}

function buyPowerupUse(id, source = 'normal') {
    const powerup = getPowerupById(id);
    if (!powerup) return false;
    const inventory = readPowerups();
    const item = inventory[id];
    if (!item?.desbloqueado) return unlockPowerup(id, source);
    const { amount, currency } = getPowerupUseAmount(powerup, source);
    if (!canAffordPowerup(amount, currency)) {
        alert(currency === 'gems' ? 'No tienes suficientes rubies.' : 'No tienes suficientes monedas.');
        return false;
    }
...
function buyPowerup(id, source = 'normal', purchaseType = 'use') {
    if (purchaseType === 'unlock') return unlockPowerup(id, source);
    return buyPowerupUse(id, source);
}

function goToVIPPowerup(id) {
    if (typeof openVIP === 'function') openVIP();
    if (typeof renderVIPPowerups === 'function') renderVIPPowerups();
    setTimeout(() => openPowerupDetail(id, 'vip'), 60);
}
...
function upgradePowerup(id) {
    ...
    const cost = powerup.upgrade[nextLevel - 1] || Math.ceil((getPowerupPricing(powerup).useGems || 8) * (1 + nextLevel * 0.8));
    ...
    refreshPowerupShopViews({ id, mode: 'vip', level: item.nivel });
    return true;
}
...
function closePowerupSelectionOverlay(onDone) {
    const overlay = document.getElementById('powerupSelectionOverlay');
    if (overlay) {
...
function showPowerupSelection(levelIndex, onDone) {
    readPowerups();
    document.body.classList.remove('is-gameplay-active', 'is-playing-touch');
    ...
    if (!Array.isArray(window.__powerupSelectionPool)) {
        window.__powerupSelectionPool = POWERUPS_DATA
            .filter(powerup => !inventory[powerup.id]?.desbloqueado)
            .sort(() => Math.random() - 0.5)
            .slice(0, owned.length ? 2 : 3)
            .map(powerup => powerup.id);
    }
    const recommended = window.__powerupSelectionPool
        .map(getPowerupById)
        .filter(powerup => powerup && !inventory[powerup.id]?.desbloqueado);
    const vipPool = getPowerupVipTeasers().filter(powerup => !inventory[powerup.id]?.desbloqueado);
    const vip = vipPool.sort(() => Math.random() - 0.5)[0] || getPowerupVipTeasers()[0] || POWERUPS_DATA[0];
    const selected = [];
    ...
    const redraw = () => {
        ...
        overlay.querySelectorAll('.powerup-pick-card').forEach(button => {
            button.onclick = () => {
                ...
                if (button.dataset.buyable) {
                    ...
                    const purchaseType = state.desbloqueado ? 'use' : 'unlock';
                    if (buyPowerup(id, 'normal', purchaseType)) {
                        window.__powerupSelectionPool = (window.__powerupSelectionPool || []).filter(pid => pid !== id);
                    }
                    redraw();
                    return;
                }
        ...
        overlay.querySelector('.powerup-vip-feature')?.addEventListener('click', () => {
            closePowerupSelectionOverlay(() => {
                goToVIPPowerup(vip.id);
            });
        });
        overlay.querySelector('#powerupSelectionStart').onclick = () => {
            setEquippedPowerups(selected);
            closePowerupSelectionOverlay(onDone);
        };
    };
...
const POWERUP_RUBY_ICON = 'assets/Imagenes/Monetizacion/Rubies.png';
const POWERUP_COIN_ICON = 'assets/Imagenes/Monetizacion/DEAD_COIN.png';

function renderPowerupPrice(powerup, source = 'normal', purchaseType = 'use') {
    const { amount, currency } = purchaseType === 'unlock'
        ? getPowerupUnlockAmount(powerup, source)
        : getPowerupUseAmount(powerup, source);
    return `${amount} ${currency === 'gems' ? 'rubies' : 'monedas'}`;
}

function renderPowerupBuyControls(id, powerup, mode, state) {
    const purchaseType = state.desbloqueado ? 'use' : 'unlock';
    const label = state.desbloqueado ? 'COMPRAR ITEM' : 'DESBLOQUEAR';
    const { amount, currency } = purchaseType === 'unlock'
        ? getPowerupUnlockAmount(powerup, mode)
        : getPowerupUseAmount(powerup, mode);
    const icon = currency === 'gems' ? POWERUP_RUBY_ICON : POWERUP_COIN_ICON;
    return `
        <div class="powerup-buy-split">
            <button class="powerup-buy-main" onclick="buyPowerup('${id}','${mode}','${purchaseType}')" type="button">${label}</button>
            <button class="powerup-buy-price" onclick="buyPowerup('${id}','${mode}','${purchaseType}')" type="button" aria-label="${amount} ${currency === 'gems' ? 'rubies' : 'monedas'}">
                <img src="${icon}" alt="">
                <span>${amount}</span>
            </button>
        </div>
...
function renderPowerupUpgradeControls(id, powerup, state) {
    const upgradeCost = powerup.upgrade[Math.max(0, (state.nivel || 1))] || 0;
    const disabled = !state.desbloqueado || state.nivel >= (powerup.maxLevel || POWERUP_MAX_LEVEL);
    return `
        <div class="powerup-buy-split powerup-upgrade-split">
            <button class="powerup-buy-main powerup-upgrade-main" onclick="upgradePowerup('${id}')" ${disabled ? 'disabled' : ''} type="button">MEJORAR</button>
            <button class="powerup-buy-price" onclick="upgradePowerup('${id}')" ${disabled ? 'disabled' : ''} type="button" aria-label="${upgradeCost} rubies">
                <img src="${POWERUP_RUBY_ICON}" alt="">
...
function syncPowerupColmenaCells() {
    document.querySelectorAll('.powerup-colmena-cell[data-powerup-id]').forEach(cell => {
        const state = getPowerupLevelState(cell.dataset.powerupId);
        const span = cell.querySelector('span');
        if (span) {
            span.textContent = state.desbloqueado
                ? `Nv.${Math.max(1, state.nivel)} · x${state.usos}`
                : 'Bloqueado';
        }
    });
}
...
function renderPowerupsShop(container, mode = 'normal', filterType = 'all') {
    ...
    let filteredPowerups = getPowerupsForShop(mode);
    if (filterType !== 'all') {
        filteredPowerups = filteredPowerups.filter(powerup => {
            const type = powerup.type || 'utilidad';
            return type === filterType;
        });
    }

    const vipTeasers = mode === 'normal' ? getPowerupVipTeasers() : [];

    const normalStage = mode === 'normal'
        ? `
        <div class="powerup-normal-shop-stage">
            <div class="powerup-normal-grid-wrap">
                <div class="powerup-shop-grid normal">
                    ${filteredPowerups.map(powerup => renderPowerupShopCard(powerup, mode)).join('')}
                </div>
                <div id="normalPowerupDetail" class="powerup-detail-slot powerup-detail-overlay"></div>
            </div>
            ${vipTeasers.length ? `
                <div class="powerup-vip-teaser-head">EXCLUSIVOS VIP</div>
                <div class="powerup-shop-grid normal powerup-vip-teaser-grid">
                    ${vipTeasers.map(powerup => renderPowerupVipTeaserCard(powerup)).join('')}
                </div>
            ` : ''}
        </div>`
        : `
        <div class="powerup-shop-grid vip colmena">
            ${filteredPowerups.map(powerup => renderPowerupShopCard(powerup, mode)).join('')}
        </div>
        <div id="vipPowerupDetail" class="powerup-detail-slot"></div>`;
...
function renderPowerupVipTeaserCard(powerup) {
    const state = getPowerupLevelState(powerup.id);
    return `
        <button class="powerup-shop-card rect powerup-vip-teaser" style="--powerup-color:${powerup.color}" onclick="goToVIPPowerup('${powerup.id}')" type="button" title="Redirigir a tienda VIP">
            <img src="assets/powerups/icons/${powerup.id}.png" alt="" onerror="this.onerror=null;this.src='${POWERUP_ICON_PLACEHOLDER}'">
            <b>${powerup.name}</b>
            <span class="powerup-vip-teaser-label">VIP · ${state.desbloqueado ? `Nv.${Math.max(1, state.nivel)}` : 'Bloqueado'}</span>
        </button>
    `;
}
...
function renderPowerupShopCard(powerup, mode) {
    ...
    return `
        <button class="powerup-shop-card ${shapeClass}" data-powerup-id="${powerup.id}" style="--powerup-color:${powerup.color}" onclick="openPowerupDetail('${powerup.id}','${mode}')" type="button">
            <img src="assets/powerups/icons/${powerup.id}.png" alt="" onerror="this.onerror=null;this.src='${POWERUP_ICON_PLACEHOLDER}'">
            <b>${powerup.name}</b>
...
function renderPowerupColmenaCell(powerup) {
    const state = getPowerupLevelState(powerup.id);
    return `
        <button class="powerup-colmena-cell" data-powerup-id="${powerup.id}" style="--powerup-color:${powerup.color}" onclick="openPowerupDetail('${powerup.id}','vip')" type="button" title="${powerup.name}">
            <img src="assets/powerups/icons/${powerup.id}.png" alt="" onerror="this.onerror=null;this.src='${POWERUP_ICON_PLACEHOLDER}'">
            <b>${powerup.name}</b>
            <span>${state.desbloqueado ? `Nv.${Math.max(1, state.nivel)} · x${state.usos}` : 'Bloqueado'}</span>
        </button>
    `;
}

function renderVIPPowerupColmenaPanel(container, banner = {}) {
    readPowerups();
    if (!container) return;
    window.currentVIPDetailRenderer = () => renderVIPPowerupColmenaPanel(container, banner);
    const bg = banner.detailBackground || banner.cover || '';
    container.innerHTML = `
        <button class="vip-back" onclick="renderVIPHome()" type="button">VOLVER A VIP</button>
        <section class="vip-reactor-hero" style="background-image:linear-gradient(90deg, rgba(2,5,12,0.82), rgba(4,10,16,0.58)), url('${bg}');">
            <div class="vip-reactor-orb"><span>BOOST</span></div>
            <div>
                <div class="vip-kicker">LABORATORIO VIP</div>
                <h2>${banner.title || 'POTENCIADORES VIP'}</h2>
                <p>Compra usos, sube niveles y desbloquea mejoras exclusivas.</p>
            </div>
        </section>
        <section class="vip-hex-lab powerup-colmena-lab" aria-label="Potenciadores VIP">
            <div class="vip-hex-links" aria-hidden="true"></div>
            <div class="powerup-colmena-grid">
                ${POWERUPS_DATA.map(renderPowerupColmenaCell).join('')}
            </div>
            <div id="vipPowerupDetail" class="powerup-detail-slot powerup-detail-overlay"></div>
        </section>
    `;
}
...
function renderVIPPowerups() {
    ...
    if (!content) return;
    const banner = (typeof VIP_PROMO_BANNERS !== 'undefined'
        ? VIP_PROMO_BANNERS.find(item => item.id === 'vip_powerups')
        : null) || { id: 'vip_powerups', title: 'POTENCIADORES VIP' };
    if (typeof renderVIPPromoDetail === 'function') {
        renderVIPPromoDetail('vip_powerups');
        return;
    }
    renderVIPPowerupColmenaPanel(content, banner);
}
...
function openPowerupDetail(id, mode = 'normal', selectedLevel = 1) {
    ...
    const currentLevel = state.desbloqueado ? Math.max(1, state.nivel || 1) : 0;
    const showVipRedirect = mode === 'normal' && isPowerupVipOnly(id);
    const showUpgrade = mode === 'vip';
    detail.innerHTML = `
        <div class="powerup-detail-floating">
            <button class="powerup-detail-close" onclick="closePowerupDetail('${mode}')" type="button" aria-label="Cerrar">×</button>
            <div class="powerup-detail-hex-shell" style="--powerup-color:${powerup.color}">
            <section class="powerup-detail-hex">
                <div class="powerup-detail-hex-glow" aria-hidden="true"></div>
                <div class="powerup-detail-kicker">POTENCIADOR</div>
                ...
                <p class="powerup-detail-desc">${selected === 1 ? powerup.base : `Nivel ${selected}: ${powerup.levels[selected - 1] || powerup.base}`}</p>
                <div class="powerup-level-row">
                    ${Array.from({ length: POWERUP_MAX_LEVEL }, (_, idx) => {
                        const level = idx + 1;
                        const disabled = level > maxLevel;
                        const classes = [
                            level === selected ? 'preview' : '',
                            level === currentLevel ? 'current' : ''
                        ].filter(Boolean).join(' ');
                        return `<button class="${classes}" onclick="openPowerupDetail('${id}','${mode}',${level})" ${disabled ? 'disabled' : ''} type="button">${level}</button>`;
                    }).join('')}
                </div>
                <div class="powerup-detail-actions">
                    ${showVipRedirect
                        ? `<button class="vip-jump" onclick="goToVIPPowerup('${id}')" type="button">Ir a tienda VIP</button>`
                        : renderPowerupBuyControls(id, powerup, mode, state)}
                    ${showUpgrade ? renderPowerupUpgradeControls(id, powerup, state) : ''}
                </div>
            </section>
            </div>
        </div>
    ...
    detail.onclick = event => {
        if (event.target === detail) closePowerupDetail(mode);
    };
    document.querySelectorAll('.powerup-colmena-cell, .powerup-normal-grid-wrap .powerup-shop-card').forEach(cell => {
        cell.classList.toggle('dimmed', cell.dataset.powerupId !== id);
    });
}

function closePowerupDetail(mode = 'vip') {
    const detail = document.getElementById(mode === 'vip' ? 'vipPowerupDetail' : 'normalPowerupDetail');
    if (!detail) return;
    detail.classList.remove('open');
    detail.innerHTML = '';
    document.querySelectorAll('.powerup-colmena-cell.dimmed, .powerup-shop-card.dimmed').forEach(cell => {
        cell.classList.remove('dimmed');
    });
}

function refreshPowerupShopViews(options = {}) {
    const { id, mode, level } = options;
    ...
    if (vip && document.getElementById('vipPowerupDetail')) {
        if (vip.querySelector('.powerup-colmena-lab')) {
            syncPowerupColmenaCells();
            if (id && mode) openPowerupDetail(id, mode, level || getPowerupLevelState(id).nivel || 1);
            return;
        }
        renderPowerupsShop(vip, 'vip');
    }
}
...
window.renderVIPPowerupColmenaPanel = renderVIPPowerupColmenaPanel;
window.openPowerupDetail = openPowerupDetail;
window.closePowerupDetail = closePowerupDetail;
window.buyPowerup = buyPowerup;
window.buyPowerupUse = buyPowerupUse;
window.unlockPowerup = unlockPowerup;
window.goToVIPPowerup = goToVIPPowerup;
...
const STARTER_PACK_POWERUPS = ['proteccion', 'fantasma', 'iman'];
...
function giveStarterPack() {
    ...
    STARTER_PACK_POWERUPS.forEach(powerupId => {
        const item = inventory[powerupId];
        if (!item) return;
        item.nivel = Math.max(1, item.nivel || 1);
        item.usos = Math.max(item.usos || 0, 3);
        item.desbloqueado = true;
    });
